...
function ResetPassword() {
  ...
  if (!token || !userId) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#c3d7e8] text-[#3E5973]">
        <p className="text-xl">Invalid or expired reset link.</p>
      </div>
    );
  }
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      toast.error(res.data.message || "Reset failed. Please try again.")
    }
  } catch (err) {
    toast.error(
